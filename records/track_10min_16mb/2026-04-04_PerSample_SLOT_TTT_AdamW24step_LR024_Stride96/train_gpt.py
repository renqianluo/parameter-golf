...
class Hyperparameters:
    ...
    ttt_lr_floor_frac = float(os.environ.get("TTT_LR_FLOOR_FRAC", "0.0"))  # min LR as fraction of peak; prevents cosine decay to ~0 for late chunks; e.g. 0.1 = floor at 10% of peak LR
    ...
    ttt_second_pass_frac = float(os.environ.get("TTT_SECOND_PASS_FRAC", "0.0"))  # if >0, do a second training pass over last N fraction of chunks at very low LR (ttt_lr*0.05); adds ~N*275s extra time; hypothesis: late chunks benefit from >1 gradient step
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_second_pass_frac > 0.0 and args.ttt_epochs > 0 and args.ttt_optimizer == "adamw":
        ...
        base_model.train()
        for ci in range(second_pass_start, num_chunks - 1):
            windows = chunk_windows[ci]
            if not windows:
                continue
            chunk_start = ci * ttt_chunk
        ...
        if rank == 0:
            elapsed = time.perf_counter() - t0
            print(f"  ttt_second_pass: chunks={num_chunks-1-second_pass_start} lr={second_pass_lr:.6f} elapsed={elapsed:.1f}s")
